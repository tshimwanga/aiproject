...
heuristique_1(Etat, Equipe, ScoreFinal) :-
    ...
    findall(ScoreAutre,
        (
            member(AutreEquipe, [bleue, rouge, vert, jaune]),
            AutreEquipe \= Equipe,
...
prochain_joueur(rouge, vert).
prochain_joueur(vert, jaune).
...
deplacement_heuristique2(etat(Lutins, Ponts), Equipe, Action) :-
    ...
    ( member(pont(X, Y, NX, NY), Ponts)
      -> PontUtilise = pont(X, Y, NX, NY)
      ;  PontUtilise = pont(NX, NY, X, Y)
    ),
    Action = deplacer_et_retirer(l(Equipe, Num), X, Y, NX, NY, PontUtilise), !.


% --- Heuristique 2 : privilégier un lutin très mobile
deplacement_heuristique(etat(Lutins, Ponts), Equipe, Action) :- 
    % Lutin ayant le plus de directions possibles (cases libres via ponts)
    findall((NbPonts, l(Equipe, Num, X, Y)),
        (
            member(l(Equipe, Num, X, Y), Lutins),
            findall(_, (
    ...
    % Trier décroissant (lutin le plus mobile en tête)
    sort(0, @>=, LutinsAvecMobilite, [(Max, l(Equipe, Num, X, Y))|_]),
    
    % Choisir un pont utilisable (vers une case libre)
    (member(pont(X, Y, NX, NY), Ponts); member(pont(NX, NY, X, Y), Ponts)),
    \+ member(l(_, _, NX, NY), Lutins),
    
    % Choisir le bon sens du pont
    ( member(pont(X, Y, NX, NY), Ponts)
      -> PontUtilise = pont(X, Y, NX, NY)
      ;  PontUtilise = pont(NX, NY, X, Y)
    ),
